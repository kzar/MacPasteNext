...
- Automatic copy diagnostics now correlate each capture's gesture, app, timing, clipboard types/change counts, and outcome. With *Show Debug Logs* enabled, bounded background accessibility probes report roles, subroles, parent roles and selection-query results at the gesture endpoints and focused element, distinguishing empty selections from unavailable attributes and errors. Exported logs include macOS, permission and capture/paste settings. These probes are diagnostic only; unavailable accessibility data does not disable copying.
- Automatic copying now skips gestures on identifiable menu/button controls, including toggle buttons and labels inside buttons, while keeping text fields inside menus selectable. Unknown or unavailable accessibility information retains the existing copy fallback. Double/triple-click selection and drag thresholds are unchanged. QuickTime Player is excluded from automatic capture because its `Cmd+C` copies video frames and can display a pasteboard error during playback.
- Pending captures are cancelled if their source app loses focus before copying, and automatic copy keystrokes are addressed to that app so opening a document cannot redirect them to another app.
...
- Middle-clicking a link or a browser tab no longer pastes over it. The click is passed through so the app can act on it: browsers open a link in a new tab and close a tab, which is what the middle button means on those everywhere else. Detected by looking for an `AXLink` or `AXRadioButton` role at or just above the click point - Firefox exposes each of its tabs as the latter - which is cheap enough to decide before the event is swallowed. Text fields are unaffected, and apps that expose no accessibility tree find nothing and paste exactly as before.
- Locally built copies no longer show an update error on launch. Sparkle refuses to start without an `SUPublicEDKey`, and that key only exists as a CI variable, so `scripts/build-release.sh` writes an empty one into every local build - which Sparkle then reported as an alert every time the app started. The updater is now only created when the bundle actually carries a key, and where it does not, the *Check for Updates* menu item is omitted and the Updates settings section explains why. Official releases ship the key and are unaffected.

- Known limitations

- Pasting at the pointer position, and passing middle-clicks through to links, both need the target app to expose an accessibility tree. Firefox does not by default - it answers a hit test with its top-level window and nothing below it, because browsers only build that tree once they detect an assistive technology - so in Firefox the paste lands at the existing cursor and a middle-click on a link pastes instead of opening it. To enable both, set `accessibility.force_disabled` to `-1` in `about:config` and restart Firefox. Be aware that this turns on Gecko's accessibility engine, which has a memory and CPU cost. Chrome and Electron apps are in the same position and have no equivalent preference.
