...
- Automatic copy diagnostics now correlate each capture's gesture, app, timing, clipboard types/change counts, and outcome. With *Show Debug Logs* enabled, bounded background accessibility probes report roles, subroles, parent roles and selection-query results at the gesture endpoints and focused element, distinguishing empty selections from unavailable attributes and errors. Exported logs include macOS, permission and capture/paste settings. These probes are diagnostic only; unavailable accessibility data does not disable copying, and there is no QuickTime-specific exclusion.
