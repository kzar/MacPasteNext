...
Automatic selection capture skips known menu/button controls and is disabled in QuickTime Player to avoid triggering video-frame copies. Text fields inside menus still support selection, and apps with incomplete accessibility information retain the existing copy fallback. This guard runs regardless of whether debug logs are shown. Double/triple-click text selection remains supported; there is no new movement threshold. Explicit `Cmd+C` and middle-click paste are unaffected.
